...
## 2026-09-18 更新日志：RK3588 拼接性能

1. 增加拼接画布上限，异常单应矩阵会在分配大图缓冲区前返回错误，避免 4 GiB 板端 OOM。
2. 对超过 300 万像素的画布，改用低内存线性融合；direct OpenCL 可用时该融合和最终锐化实际运行在 Mali-G610 GPU。
3. 大图不再执行 CPU 非局部去噪；超过 150 万像素的输入也跳过 CPU 双边滤波。
4. ORB 在最长边 960 像素的匹配副本上计算，随后按各轴实际比例把关键点坐标映射回 1920 宽工作图再估计单应矩阵，输出尺寸与 GPU warp 不降采样。
5. 板端实测基准输入为 `P1060373.JPG` / `P1060374.JPG`（各 2560×1920）：GPU 融合前总耗时 42.0 秒；启用 GPU 融合、GPU 锐化和跳过去噪后为 1.43 秒。ORB/BFMatcher/RANSAC 仍走 CPU，原因是板端 OpenCV 4.5.4 未启用 OpenCL (`haveOpenCL=False`)；这是当前剩余的主要 CPU 部分。
